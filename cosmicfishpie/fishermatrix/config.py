...
def init(options=dict(),
    ...
    settings.setdefault(
        ...
        os.path.join(
            ...
            '..',
            '..',
            'boltzmann_yaml_files'))
    settings.setdefault(
        'class_config_yaml',
        os.path.join(
            settings['boltzmann_yaml_path'],
            'class',
            'default.yaml'))
    settings.setdefault(
        'camb_config_yaml',
    ...
    if extfiles is not None and settings['code'] == 'external':
        ...
        if os.path.isdir(external['directory']):
            ...
            for dd in external['folder_paramnames']:
                ...
                if lensub < 1:
                    raise ValueError(
                        "External directory does not contain appropriate subfolders for:  {:s} ".format(dd))
                else:
                    print("External directory: ", external['directory'])
                    print(
                        "{:d} subfolders for parameter {:s}".format(
                            lensub, dd))
        else:
        ...
        parsed_boltzmann = yaml.load(
            boltzmann_yaml_file, Loader=yaml.FullLoader)
        ...
        if not isinstance(biaspars, dict):
            if biasmodel == 'sqrt':
                ...
                for ind in range(1, len(zbins)):
                    key = 'b' + str(ind)
                    biaspars[key] = np.sqrt(
                        1 + 0.5 * (zbins[ind] + zbins[ind - 1]))
            else:
