...
def faster_integral_efficiency(i, ngal_func, comoving_func, zarr):
    zprime = zarr[:, None]
    wintgd = ngal_func(zprime, i) * \
        (1.0 - comoving_func(zarr) / comoving_func(zprime))
...
class ComputeCls:
    ...
    def __init__(self, cosmopars,
        ...
        if 'GCph' in self.observables and 'WL' in self.observables:
            cfg.specs['ellmax'] = max(
                cfg.specs['lmax_GCph'], cfg.specs['lmax_WL'])
            cfg.specs['ellmin'] = min(
                cfg.specs['lmin_GCph'], cfg.specs['lmin_WL'])
        ...
        if cfg.input_type == 'class':
            ...
            self.z_min = np.max(
                [cfg.specs['z_bins'][0], self.cosmo.results.zgrid[0]])
            # self.z_max = np.min([cfg.specs['z_bins'][-1]+0.5, self.cosmo.results.zgrid[-1]])
            self.z_max = np.min(
                [cfg.specs['z_bins'][-1], self.cosmo.results.zgrid[-1]])
    ...
    def P_limber(self):
        ...
        for ell, zin in product(range(self.ellsamp), range(self.zsamp)):
            ...
            if kappa <= cfg.specs['kmax']:
                self.Pell[zin,
                          ell] = (self.cosmo.SigmaMG(self.z[zin],
                                                     kappa)**2) * self.cosmo.matpow(self.z[zin],
                                                                                    kappa,
                                                                                    nonlinear=cfg.settings['nonlinear_photo']) / (chi[zin]**2.)
            else:
                self.Pell[zin, ell] = 0.
        return None
    ...
    def sqrtP_limber(self):
        ...
        for ell, zin in index_pknn:
            self.sqrtPell['WL'][zin,
                                ell] = (self.cosmo.SigmaMG(self.z[zin],
                                                           kappa[ell,
                                                                 zin]) * np.sqrt(self.cosmo.matpow(self.z[zin],
                                                                                                   kappa[ell,
                                                                                                         zin],
                                                                                                   nonlinear=cfg.settings['nonlinear']))) / chi[zin]
            # Sakr Fix June 2023
            self.sqrtPell['WL_IA'][zin,
                                   ell] = 1.0 * np.sqrt(self.cosmo.matpow(self.z[zin],
                                                                          kappa[ell,
                                                                                zin],
                                                                          nonlinear=cfg.settings['nonlinear'])) / chi[zin]
            self.sqrtPell['GCph'][zin,
                                  ell] = np.sqrt(self.cosmo.matpow(self.z[zin],
                                                                   kappa[ell,
                                                                         zin],
                                                                   nonlinear=cfg.settings['nonlinear'],
                                                                   tracer=self.tracer)) / chi[zin]
        return None
    ...
    def galaxy_kernel(self, z, i):
       ...
       # Wgc = self.window.norm_ngal_photoz(z,i) * self.nuisance.bias(self.biaspars, i)(z) * \
        ...
        if self.feed_lvl >= 3:
            print(
                '    ...done bin {} for GCph in {:.2f} s'.format(
                    i, tgcend - tgcstart))
    ...
    def lensing_kernel(self, z, i):
        ...
        prefac = (3. / 2.) * self.cosmo.Hubble(0.)**2. * \
            self.cosmo.Omegam_of_z(0.)
        ...
        if self.feed_lvl >= 3:
            print(
                '    ...done bin {} for WL in {:.2f} s'.format(
                    i, twlend - twlstart))
    ...
    def lensing_efficiency(self):
        ...
        if self.feed_lvl >= 3:
            print(
                '    ...lensing efficiency computation took {:.2f} s'.format(
                    teffend - teffstart))
        return efficiency
            ...
            self.GC = [
                interp1d(
                    ...
                    self.galaxy_kernel(
                        self.z,
                        ind),
                    kind='cubic') for ind in self.binrange]
    ...
    def computecls(self):
        ...
        for obs1, obs2, bin1, bin2 in product(
            ...
            for ind, lval in enumerate(full_ell):
                if (cfg.specs['lmin_' +
                              obs1] <= lval <= cfg.specs['lmax_' +
                                                         obs1]) and (cfg.specs['lmin_' +
                                                                               obs2] <= lval <= cfg.specs['lmax_' +
                                                                                                          obs2]):
                    finalcls[ind] = clinterp(lval)

            cls[obs1 + ' ' + str(bin1) + 'x' + obs2 +
                ' ' + str(bin2)] = finalcls
        ...
        if self.feed_lvl > 1:
            print(
                '    Cls integral computation took {:.2f} s'.format(
                    tend - tstart))

        return cls

    def clsintegral(self, obs1, obs2, bin1, bin2, hub):
        mask1 = ((self.ell >= cfg.specs['lmin_' + obs1])
                 & (self.ell <= cfg.specs['lmax_' + obs1]))
        mask2 = ((self.ell >= cfg.specs['lmin_' + obs2])
                 & (self.ell <= cfg.specs['lmax_' + obs2]))
        ...
        intgn = (self.sqrtPell[obs1] * self.genwindow(self.z,
                                                      obs1,
                                                      bin1)[0][:,
                                                               np.newaxis] / np.sqrt(hub)[:,
                                                                                          np.newaxis] + self.sqrtPell[obs1 + '_IA'] * self.genwindow(self.z,
                                                                                                                                                     obs1,
                                                                                                                                                     bin1)[1][:,
                                                                                                                                                              np.newaxis] / np.sqrt(hub)[:,
                                                                                                                                                                                         np.newaxis]) * (self.sqrtPell[obs2] * self.genwindow(self.z,
                                                                                                                                                                                                                                              obs2,
                                                                                                                                                                                                                                              bin2)[0][:,
                                                                                                                                                                                                                                                       np.newaxis] / np.sqrt(hub)[:,
                                                                                                                                                                                                                                                                                  np.newaxis] + self.sqrtPell[obs2 + '_IA'] * self.genwindow(self.z,
                                                                                                                                                                                                                                                                                                                                             obs2,
                                                                                                                                                                                                                                                                                                                                             bin2)[1][:,
                                                                                                                                                                                                                                                                                                                                                      np.newaxis] / np.sqrt(hub)[:,
                                                                                                                                                                                                                                                                                                                                                                                 np.newaxis])
