...
Status: resolved diagnosis for the photometric gate failures in cases 09, 11,
13, and 15. Supersedes the "nonlinear HMcode2020 baseline difference" reading
in `CASE13_W0WA_MNU_INVESTIGATION.md`, which was directionally right but wrong
by an order of magnitude about the size of the backend effect.

Source data: case 15 (`compare_photo_camb_vs_class_cfg_ea069c683b`) and case 16
...
## Case 15 unmarginalized versus marginalized errors
...
Marginalizing case 15's own matrices over nested subsets of free parameters,
...
## Contrast with the spectroscopic case 16

Case 16 is *more* ill-conditioned than case 15, yet agrees far better:

| quantity | case 15 photo | case 16 spectro |
|---|---:|---:|
...
The 3809.9s CLASS runtime for case 16 is the price of that agreement:
...
1. The dominant degenerate block is **not** `mnu`-`Neff`-`w0`-`wa`. Case 15 has
   `Neff` fixed and is *worse* than case 13 (14.71% versus 12.82%). The
...
09 (6.82%), 11 (10.06%), 13 (12.82%), and 15 (14.71%): every additional free
parameter that projects onto the amplitude direction moves the matrix closer to
...
3. Make the blocking gate act on the unmarginalized deviation, keeping the
   marginalized deviation as informational. Under that criterion case 15 passes
...
4. Test the residual 1.55% with a photometric run using the spectroscopic CLASS
   ...
   `P_k_max_1/Mpc: 50`). Expect a CLASS runtime comparable to case 16.
