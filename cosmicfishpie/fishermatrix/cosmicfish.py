...
class FisherMatrix:
    ...
    def compute(self, max_z_bins=None):
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                       ...
                       time_ini=tfishstart,
                       time_fin=tfishend)
        fishMat_object = self.export_fisher(
            finalFisher, totaltime=tfishend - tfishstart)
    ...
    def set_pk_settings(self):
        ...
        self.Pk_kmesh, self.Pk_mumesh = np.meshgrid(
            self.Pk_kgrid, self.Pk_mugrid)
    ...
    def pk_LSS_Fisher(self,
        ...
        upt.time_print(
            feedback_level=self.feed_lvl,
            min_level=1,
            text="Fisher Matrix shape: {}".format(
                fisherMatrix.shape),
            instance=self)
    ...
    def photo_LSS_fishermatrix(
                    ...
                    if ind1 > ind2:
                        ...
                        for ia, aa in enumerate(cols):
                            for ib, bb in enumerate(cols):
                                der1[ia,
                                     ib] = derivs[par1][aa + 'x' + bb][i_ell]
                                der2[ia,
                                     ib] = derivs[par2][aa + 'x' + bb][i_ell]
        ...
        upt.time_print(
            feedback_level=self.feed_lvl,
            min_level=0,
            text='Finished calculation of Fisher Matrix for {} in: '.format(
                self.observables),
            time_ini=tini,
            time_fin=tfin)
        return FisherVV

    def CMB_fishermatrix(
            self,
            noisy_cls=None,
            covmat=None,
            derivs=None,
            ...
            for ind2, par2 in enumerate(self.freeparams):
                if ind1 > ind2:
                    ...
                    for i_ell in range(len(lvec)):
                        ...
                        for obs1, obs2 in product(
                                self.observables, self.observables):
                            der1.at[obs1,
                                    obs2] = derivs[par1][obs1 + 'x' + obs2][i_ell]
                            der2.at[obs1,
                                    obs2] = derivs[par2][obs1 + 'x' + obs2][i_ell]
                        ...
                        Fisher[ind1, ind2] = Fisher[ind1, ind2] + \
                            (trace * (lvec[i_ell] + 0.5))

                    tparend = time()
                    upt.time_print(
                        feedback_level=self.feed_lvl,
                        min_level=1,
                        text='Fisher entry ({}, {}) done in '.format(
                            par1,
                            par2),
                        time_ini=tparstart,
                        time_fin=tparend)
        ...
        upt.time_print(
            feedback_level=self.feed_lvl,
            min_level=0,
            text='Finished calculation of Fisher Matrix for {}'.format(
                self.observables))
            ...
            if '_' in key:
                ...
                if ibin > nmax:
                    ...
                    upt.time_print(
                        feedback_level=self.feed_lvl,
                        min_level=1,
                        text="Removed free param: {:s} = {:s}".format(
                            key,
                            removedpar),
                        instance=self)
                else:
    ...
    def export_fisher(self, fishmat, totaltime=None):
        ...
        if cfg.settings['outroot'] != '':
            ...
            upt.time_print(
                feedback_level=self.feed_lvl,
                min_level=0,
                text="Fisher matrix exported: {:s}".format(
                    filename + extension))
