...
matplotlib.rcParams['text.usetex'] = False  # use latex for all text handling. The following fonts
#matplotlib.rcParams['text.latex.preamble'] =  '\usepackage{amsmath},\usepackage{pgfplots},\usepackage[T1]{fontenc}'
matplotlib.rcParams['text.antialiased'] = True # If True (default), the text will be antialiased.
...
def og_plot_shades(ax, x_arr, x_names, lighty_arr=None, darky_arr=None, mats_labels=None,
    ...
    if plotlight==True and lighty_arr is None:
        print("Error: plotlight is True but lighty_arr is None. Aborting.")
        return None
    if plotdark==True and darky_arr is None:
    ...
    for ii, lbl in enumerate(mats_labels):
        if plotlight:
            ax.plot(x_arr, lighty_arr[ii,:], marks[ii], c=cols[ii], ms=LW*8, alpha=aalpha, label=lbl)
        if plotdark:
            if plotlight==False:
                ms=LW*8; lbl=lbl
            else:
                ms=0; lbl=None
            ax.plot(x_arr, darky_arr[ii, :], marks[ii], c=cols[ii], ms=ms, mew=2, alpha=aalpha, label=lbl)
    if plotlight:
...
def plot_shades(ax, x_arr, x_names, lighty_arr=None, darky_arr=None, mats_labels=None,
    ...
    for ii, lbl in enumerate(mats_labels):
        if plotlight:
            ax.scatter(x_arr, lighty_arr[ii,:], color=cols[ii], marker=marks[ii], s=(LW*8)**2, label=lbl, alpha=aalpha,
                       zorder=3+ii)
        if plotdark:
            if plotlight==False:
                #ms=LW*12; lbl=lbl
                #ax.scatter(x_arr, darky_arr[ii], color=cols[ii], marker=marks[ii], s=ms, label=lbl)
                ax.scatter(x_arr, darky_arr[ii,:], color=cols[ii], marker=marks[ii], s=(LW*8)**2, label=lbl, 
                           alpha=aalpha-0.1,
                           zorder=3+ii)
            else:
                ms=0; lbl=None
...
def process_fish_errs(fishers_list, fishers_name, parstoplot=None,
                      parsnames_latex=None,
                      marginalize_pars=True, print_errors=True, transform_latex_dict=dict()):
...
def ploterrs(fishers_list, fishers_name, parstoplot=None, parsnames_latex=None,
    ...
    if plot_style == 'original':
        og_plot_shades(ax1, x_pars, parsnames_latex, mats_labels=fishers_name, lighty_arr=emrel, darky_arr=eurel,
                lightdark_names=['marg.','unmarg.'],
                plotlight=plot_marg, plotdark=plot_unmarg,
                fish_leg_loc=fish_leg_loc, yrang=yrang, y_label=y_label
                ,
        ...
        plot_shades(ax1, x_pars, parsnames_latex, mats_labels=fishers_name, lighty_arr=emrel, darky_arr=eurel,
                lightdark_names=['marg.','unmarg.'],
                plotlight=plot_marg, plotdark=plot_unmarg,
                fish_leg_loc=fish_leg_loc, yrang=yrang, y_label=y_label
                ,
...
def matrix_plot(matrix, xlabel='Ratio', ticklabels=None,
                filename='matrixplot.png', figsize=(9,9), colormap=plt.cm.viridis,
                savefig=True, dpi=200):
