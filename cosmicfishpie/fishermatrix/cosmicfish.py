...
class FisherMatrix:
    ...
    def __init__(self,
         ...
         cosmoModel='w0waCDM',
         latexnames=None,
         parallel='False', parallel_cpus=4):
        ...
        self.allparams_fidus = {**self.fiducialcosmopars, **self.photopars,
                                **self.IApars, **self.biaspars,
                                **self.Spectrobiaspars, **self.IMbiaspars, 
                                **self.PShotpars}
    ...
    def compute(self, max_z_bins=None):
        ...
        if 'GCph' in self.observables or 'WL' in self.observables:
            self.photo_obs_fid = photo_obs.ComputeCls(self.fiducialcosmopars, self.photopars, 
                                                self.IApars, self.biaspars,
                                                print_info_specs=True)
            self.photo_LSS = photo_cov.PhotoCov(self.fiducialcosmopars, self.photopars, 
                                                self.IApars, self.biaspars,
                                                fiducial_Cls=self.photo_obs_fid)
            noisy_cls, covmat = self.photo_LSS.compute_covmat()
            derivs = self.photo_LSS.compute_derivs()
            photoFM = self.photo_LSS_fishermatrix(noisy_cls=noisy_cls,
                                                 covmat=covmat, derivs=derivs)
            ...
            if 'IM' in self.observables and 'GCsp' in self.observables:
                ...
                self.pk_obs = spec_obs.ComputeGalIM(cosmopars=self.fiducialcosmopars,
                ...
                self.pk_obs = spec_obs.ComputeGalSpectro(cosmopars=self.fiducialcosmopars,
                                                fiducial_cosmopars=self.fiducialcosmopars,
                                                bias_samples=self.obs_spectrum,
                                                spectrononlinearpars=self.Spectrononlinpars)
                
            self.pk_cov = spec_cov.SpectroCov(self.fiducialcosmopars, 
            ...
            for ibi in range(max_z_bins):
                ...
                if 'IM' in self.observables and 'GCsp' in self.observables:
                    self.veff_arr.append(self.pk_cov.veff_XC(ibi, k_mesh, mu_mesh))##compute  in the case of IMxGC 
            ...
            upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                    ...
                    time_ini=tvi, time_fin=tvf, instance=self)
        ...
        elif 'CMB_T' in self.observables or 'CMB_E' in self.observables or 'CMB_B' in self.observables:
            ...
            CMB_FM = self.CMB_fishermatrix(noisy_cls=noisy_cls,
                                           covmat=covmat, derivs=derivs)
    ...
    def set_pk_settings(self):
        ...
        self.Pk_kgrid = np.linspace(self.kmin_fish,
                                    self.kmax_fish,
                                    self.Pk_ksamp)
        self.Pk_mugrid = np.linspace(0., 1., self.Pk_musamp)
        self.Pk_kmesh, self.Pk_mumesh = np.meshgrid(self.Pk_kgrid, self.Pk_mugrid)

    def compute_binned_derivs(self, z_arr):
        self.pk_deriv = spec_cov.SpectroDerivs(z_arr, 
                                                self.Pk_kmesh,
                                                self.Pk_mumesh,
    ...
    def pk_LSS_Fisher(self,
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                       ...
                       instance=self)
        upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                       text='k_min = {:.3f}Mpc^-1, k_max = {:.3f}Mpc^-1,'.format(self.kmin_fish, self.kmax_fish),
                       instance=self)
        upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                       text="Fisher Matrix shape: {}".format(fisherMatrix.shape),
                       instance=self)
        fisherMatrix = np.array([self.fisher_per_bin(ibin) 
            ...
            for ind1,par1 in enumerate(self.freeparams):
                for ind2,par2 in enumerate(self.freeparams):
                    if ind1>ind2:
                        ...
                        mat3   = der2.dot(mat2)
                        trace  = np.trace(mat3)
                        FisherV[i_ell, ind1, ind2] = (trace*(lvec_ave[i_ell]+0.5)*delta_ell[i_ell])
                    if i_ell==1: 
                        tparend = time()
                        upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                                    text='FisherV entry ({}, {}) for ell index {} done in '.format(par1, par2, i_ell),
                                    time_ini=tparstart,
                                    time_fin=tparend)
            ...
            for ind2,par2 in enumerate(self.freeparams):
                if ind1>ind2:
                    ...
                    for i_ell in range(len(lvec)):
                        ...
                        der1 = der1.fillna(0.) # with 0s rather than NaNs

                        der2 = pd.DataFrame(index=cols, columns=cols)
                        der2 = der2.fillna(0.) # with 0s rather than NaNs
                        ...
                        invMat = pd.DataFrame(np.linalg.pinv(covdf.values), covdf.columns, covdf.index)
                        mat1   = der1.dot(invMat)
                        mat2   = invMat.dot(mat1)
                        mat3   = der2.dot(mat2)
    ...
    def eliminate_zbinned_freepars(self, nmax):
        ...
        for key in freepars.keys():
            if '_' in key:
                keysplit = key.split('_')  ##convention for z-binned pars: par_ibin
                ibin = int(keysplit[1])
                if ibin > nmax:
    ...
    def export_fisher(self, fishmat, totaltime=None):
        ...
        if cfg.settings['outroot'] != '':
            ...
            filename = (cfg.settings['results_dir'] +
                        '/'+FisherMatrix.cf_version+'_'+cfg.settings['outroot'] +
                        '_'+obstring)
            ...
            with open(filename+'.paramnames', 'w') as f:
                ...
                for par in self.freeparams.keys():
                    f.write(str(par)+'    '+str(cfg.latex_names.get(par, str(par)))+'    '+'{:.6f}'.format(self.allparams[par]))
                    f.write('\n')
            ...
            with open(specs_name, 'w') as f:
                ...
                if totaltime is not None:
                    f.write('**Time needed for computation of Fisher matrix: {:.3f} seconds \n'.format(totaltime))
            ...
            print()
            upt.time_print(feedback_level=self.feed_lvl, min_level=1,
                       text="CosmicFish settings and Survey specifications exported: {:s}".format(specs_name),
                       instance=self)
            return fishMat_obj
