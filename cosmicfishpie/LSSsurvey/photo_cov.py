...
class PhotoCov:
    ...
    def getcls(self, allpars):
        ...
        if cosmopars==self.cosmopars and self.fiducial_Cls is not None:
            cls = photo_obs.ComputeCls(cosmopars, photopars, IApars, biaspars,
    ...
    def getclsnoise(self,cls):
        ...
        ngalbin = np.array(cfg.specs['ngalbin']) #TBA: reading the number directly. This should be computed
        ...
        for ind in self.binrange:
            for obs in self.observables:
                if obs == 'GCph':
                    noisy_cls[obs+' '+str(ind)+'x'+obs+' '+str(ind)] += 1./ngalbin[ind-1] #TBA: fix this 
                elif obs == 'WL':
                    noisy_cls[obs+' '+str(ind)+'x'+obs+' '+str(ind)] += (ellipt_error**2.)/ngalbin[ind-1]
    ...
    def get_covmat(self,noisy_cls):
        ...
        for ind,ell in enumerate(noisy_cls['ells']):
            ...
            for obs1,obs2,bin1,bin2 in product(self.observables, self.observables, self.binrange, self.binrange):
                covdf.at[obs1+' '+str(bin1),obs2+' '+str(bin2)] = (noisy_cls[obs1+' '+str(bin1)+'x'+obs2+' '+str(bin2)][ind]/
