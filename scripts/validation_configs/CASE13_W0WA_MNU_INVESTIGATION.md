# Case `08.2.0` (`w0waCDM + mnu + Neff`, formerly "case 13") investigation

> **Superseded in part.** Cases `07.2.0` and `07.1.0` (formerly "case 15"/"case 16") have
...
Case `08.2.0` is a photometric CAMB-vs-CLASS comparison with nonlinear HMcode2020
...
- The spectroscopic counterpart (case `08.1.0`) has only a 0.89% `mnu` deviation,
...
Case `07.2.0` (formerly "case 15") isolates the effect of freeing `Neff` without
changing the `08.2.0` photo settings. It uses the same fiducial parameters, options,
...
```bash
uv run bash scripts/run_selected_validations.sh --cases 07.2.0 --omp-threads 8 --force
```

The runner also includes `07.2.0` in `--all`. The run is intentionally not
started by this change because it is an expensive Fisher validation.

## Outcome

Case `07.2.0` was run and gives a marginalized `mnu` deviation of 14.71%, which is
*worse* than `08.2.0` despite `Neff` being fixed, and a `sigma8` deviation of
...
The unmarginalized errors resolve it. Case `07.2.0`'s CAMB and CLASS Fisher matrices
...
contrast with spectroscopic case `07.1.0`, and the recommended changes to the gate
