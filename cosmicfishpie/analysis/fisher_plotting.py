...
class fisher_plotting:
    ...
    def __init__(self, **options):
        ...
        if self.fish_files is not None:
            ...
            for fmat in self.fishers_list:
                if type(fmat) is not fm.fisher_matrix:
                    raise TypeError("Fisher matrix in list is not of the correct type fm.fishermatrix")
        ...
        if self.fishers_group is not None:
            if type(self.fishers_group) is not fpa.CosmicFish_FisherAnalysis:
    ...
    def param_limits_bounds(self, axis_custom_factors=None):
        ...
        for par in self.paramnames_0:
            self.param_lims_bounds[par]=[centers[par]-onesigmas[par]*factors_def[par], 
    ...
    def plot_fisher(self, **kwargs):
        ...
        g.triangle_plot(self.gaussians, self.plot_pars,
                        filled=filled_,
    ...
    def compare_errors(self, options=dict()):
        ...
        pc.ploterrs(self.fishers_group.get_fisher_list(),
                    self.fish_labels,
                    ...
                    figsize=figsize, 
                    dpi=dpi, 
                    savefig=True, y_label=r'% discrepancy on $\sigma_i$ w.r.t. median',
    ...
    def matrix_ratio(self, r_fishers_list=None, 
        ...
        pc.matrix_plot(ratio_mat, xlabel=plot_title, ticklabels=tick_labels,
                filename=filename, figsize=(9,9), colormap=plt.cm.viridis,
                savefig=savefig, dpi=200) 
