...
#                noisy_cls[obs+'x'+obs] += 1/pol_Noise
        ...
        thetab        = [arcmin_to_rad*beam/np.sqrt(8.*np.log(2.)) for beam in cfg.specs['CMB_fwhm']]

        Bell          = [np.exp(ang**2.*noisy_cls['ells']*(noisy_cls['ells']+1)/2.) for ang in thetab]

        wtemp         = [(arcmin_to_rad*cfg.specs['CMB_fwhm'][ind]*cfg.specs['CMB_temp_sens'][ind])**(-2.) for ind in range(len(cfg.specs['CMB_fwhm']))]
        wpol          = [(arcmin_to_rad*cfg.specs['CMB_fwhm'][ind]*cfg.specs['CMB_pol_sens'][ind])**(-2.) for ind in range(len(cfg.specs['CMB_fwhm']))]
        ...
        if len(cfg.specs['CMB_fwhm'])>1:
            TTnoise  = np.array([self.sum_inv_squares(TTnoise_chan[:,ind]) for ind in range(len(noisy_cls['ells']))])
            polnoise = np.array([self.sum_inv_squares(polnoise_chan[:,ind]) for ind in range(len(noisy_cls['ells']))])
        else:
    ...
    def compute_covmat(self):
        ...
        if cfg.settings['feedback'] > 0: print('Computing fiducial')
        ...
        if cfg.settings['feedback'] > 0: print('')
        if cfg.settings['feedback'] > 0: print('Fiducial generated in {:.2f} s'.format(t2-t1))
        ...
        if cfg.settings['feedback'] > 0: print('')
        if cfg.settings['feedback'] > 0: print('Noise added to fiducial')
        t2  = datetime.datetime.now().timestamp()
        if cfg.settings['feedback'] > 0: print('')
        if cfg.settings['feedback'] > 0: print('Noisy Cls generated in {:.2f} s'.format(t2-t1))
        ...
        if cfg.settings['feedback'] > 0: print('')
        if cfg.settings['feedback'] > 0: print('Computed covariance matrix')
        t2  = datetime.datetime.now().timestamp()
        if cfg.settings['feedback'] > 0: print('')
        if cfg.settings['feedback'] > 0: print('Covmat of Cls generated in {:.2f} s'.format(t2-t1))

        tfin  = datetime.datetime.now().timestamp()
        if cfg.settings['feedback'] > 0: print('')
        if cfg.settings['feedback'] > 0: print('Total calculation in {:.2f} s'.format(tfin-tini))
