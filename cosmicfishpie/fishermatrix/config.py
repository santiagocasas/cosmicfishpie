...
def init(options=dict(),
    ...
    settings.setdefault('specs_dir', os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..','survey_specifications'))
    ...
    settings.setdefault('Pshot_nuisance_fiducial', 0.)
    settings.setdefault('pivot_z_IA', 0.)
    ...
    settings.setdefault('boltzmann_yaml_path', os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..','boltzmann_yaml_files'))
    settings.setdefault('class_config_yaml', os.path.join(settings['boltzmann_yaml_path'],'class','default.yaml'))
    settings.setdefault('camb_config_yaml', os.path.join(settings['boltzmann_yaml_path'],'camb','default.yaml'))
    ...
    if extfiles is not None and settings['code']=='external':
        ...
        if os.path.isdir(external['directory']):
            ...
            lendir = len(fidudir)
            if lendir < 1:
                raise ValueError("External directory does not contain fiducial folder:  {:s} ".format(dd))
            for dd in external['folder_paramnames']:
                subdirs = glob.glob(os.path.join(external['directory'], dd+"*"))
                lensub = len(subdirs)
                if lensub < 1:
                    raise ValueError("External directory does not contain appropriate subfolders for:  {:s} ".format(dd))
                else:
    ...
    if 'Euclid' in surveyName:
        ...
        specificationsf['ngalbin'] = ngal_per_bin(specificationsf['ngal_sqarmin'],
                                                  specificationsf['z_bins'])
        ...
        specificationsf['binrange'] = range(1,len(specificationsf['z_bins']))
        specificationsf['survey_name'] = surveyName
    elif surveyName=='SKA1-pessimistic':  ##needs to be modified to account for pessimistic and cross
    ...
    #     specificationsf['IM_THI_noise_file'] = 'SKA1_THI_sys_noise.txt'
        ...
        print("Other named survey specifications not implemented yet.",
    ...
    if freepars is None:
        ...
        if settings['cosmo_model']=='w0waCDM':
            ...
            print("Other cosmological models not implemented yet.",
    ...
    tcosmo1=time()
    fiducialcosmo = cosmology.cosmo_functions(fiducialparams, 
                                               input_type)
    tcosmo2 = time()
    upt.time_print(feedback_level=feed_lvl, min_level=1,
                    text='---> Cosmological functions obtained in: ',
                    time_ini=tcosmo1, time_fin=tcosmo2 )

    global biasparams
    if 'GCph' in obs:
        if biaspars is None:
            biasmodel = specs['bias_model']
        elif type(biaspars)==str:
            biasmodel = biaspars
        if type(biaspars) != dict:
            if biasmodel=='sqrt':
                biaspars = {'bias_model': 'sqrt',
                            'b0': 1.}
            elif biasmodel=='flagship':
                biaspars = {'bias_model': 'flagship',
                            'A': 1.,
                            'B': 2.5,
                            'C': 2.8,
                            'D': 1.6}
    ...
    if photopars is None:
        ...
        photopars = {'fout': 0.1,   ## does this need to be updated for SKA1??
                     'co': 1,
                     'cb': 1,
                     'sigma_o': 0.05,
                     'sigma_b': 0.05,
    ...
    if Spectrobiaspars is not None:
        ...
        if 'GCsp' in obs:
            ...
            for ii in range(1,Nbin+1):
                bstr_i, b_i = nuis.bterm_z_key(ii, z_mids, 
                               fiducialcosmo, bias_sample)
