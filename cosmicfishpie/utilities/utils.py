...
class inputiniparser():
    def __init__(self, config_dir, config_file='main.ini', parameter_translator=dict(), fiducial_translator=dict()):
        ...
        self.main_fiducials= [fiducial_translator.get(ii, 1.0)*float(pars_cosmo_dict[ii]) for ii in self.main_paramnames]
        print("fiducials")
        print(self.main_fiducials)
        self.main_paramnames = [translator.get(kk, self.mirror_paramnames[kk]) for kk in self.main_paramnames]
    ...
    def free_epsilons(self, epsilon=0.01):
        main_precision_epsilons = self.configmain.get('params_precision', 'abs_epsilons')#list[main_precision_dict['abs_epsilons']]
        ...
        self.main_freepars_dict = dict(zip(self.main_paramnames, len(self.main_paramnames)*[epsilon]))
        return None