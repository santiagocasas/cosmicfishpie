...
#@memory.cache
def memo_integral_efficiency(i, ngal_func, comoving_func, z,
                             zint_mat, diffz):
    intg_mat=np.array([(ngal_func(zint_mat[zii], i)*
...
class ComputeCls:
    ...
    def __init__(self, cosmopars, 
        ...
        self.observables = []
        for key in cfg.obs:
            if key in ['GCph','WL']: self.observables.append(key)
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=2,
                       ...
                       instance=self, time_ini=tnuis1, time_fin=tnuis2)
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=2,
                       ...
                       instance=self, time_ini=tngal1, time_fin=tngal2)
        ...
        self.ell      = np.logspace(np.log10(cfg.specs['ellmin']),
                                    np.log10(cfg.specs['ellmax']+10),
                                    num=self.ellsamp)
        ...
        self.z        = np.linspace(self.z_min,self.z_max,self.zsamp) #+1 to go beyond the bin limit
    ...
    def compute_all(self):
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=2,
                       ...
                       time_ini=tplim1, time_fin=tplim2, instance=self)
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=2,
                       ...
                       time_ini=tkern1, time_fin=tkern2, instance=self)
        ...
        upt.time_print(feedback_level=self.feed_lvl, min_level=2,
                       ...
                       time_ini=tcls1, time_fin=tcls2, instance=self)
        tend = time()
    ...
    def P_limber(self):
        ...
        for ell,zin in product(range(self.ellsamp),range(self.zsamp)):
            ...
            if kappa <= cfg.specs['kmax']:
                self.Pell[zin,ell] = (self.cosmo.SigmaMG(self.z[zin], kappa)**2) * \
                    self.cosmo.matpow(self.z[zin],kappa,nonlinear=cfg.settings['nonlinear_photo'])/(chi[zin]**2.)
            else:
                self.Pell[zin,ell] = 0.
        return None
    ...
    def sqrtP_limber(self):
        ...
        for ell,zin in index_pknn:
            self.sqrtPell['WL'][zin,ell] = (self.cosmo.SigmaMG(self.z[zin], kappa[ell,zin]) * \
            np.sqrt(self.cosmo.matpow(self.z[zin],kappa[ell,zin],nonlinear=cfg.settings['nonlinear'])))/chi[zin]
            # Sakr Fix June 2023
            self.sqrtPell['WL_IA'][zin,ell] = 1.0 * \
            np.sqrt(self.cosmo.matpow(self.z[zin],kappa[ell,zin],nonlinear=cfg.settings['nonlinear']))/chi[zin] 
            self.sqrtPell['GCph'][zin,ell] = np.sqrt(self.cosmo.matpow(self.z[zin],kappa[ell,zin],nonlinear=cfg.settings['nonlinear'],tracer=self.tracer))/chi[zin]
        return None
    ...
    def galaxy_kernel(self,z,i):
        ...
        #Wgc = self.window.norm_ngal_photoz(z,i) * np.array([self.biaspars['b{:d}'.format(i)] * \
       ...
       # Wgc = self.window.norm_ngal_photoz(z,i) * self.nuisance.bias(self.biaspars, i)(z) * \
        ...
        tgcend = time()
        if self.feed_lvl >= 3:
            print('    ...done bin {} for GCph in {:.2f} s'.format(i,
                                                               tgcend-tgcstart))
    ...
    def lensing_kernel(self,z,i):
        ...
        prefac = (3./2.)*self.cosmo.Hubble(0.)**2.*self.cosmo.Omegam_of_z(0.)
        #WARNING! this prefactor needs to be generalized (Sigma, non CDM...)
        Wwl = np.array([prefac*(1+zi) * \
                       self.cosmo.comoving(zi) * \
                       self.efficiency[i](zi) for zi in z])

        #Adding Intrinsic alignment
        WIA = self.window.norm_ngal_photoz(z,i) * \
              np.array([self.IAvalue(zi) * \
                       self.cosmo.Hubble(zi) for zi in z])
        # Sakr Fix June 2023               
        #kernel = Wwl+WIA
        
        twlend = time()

        if self.feed_lvl >= 3: print('    ...done bin {} for WL in {:.2f} s'.format(i,twlend-twlstart))
    ...
    def lensing_efficiency(self):
        ...
        teffend = time()
        if self.feed_lvl >= 3: print('    ...lensing efficiency computation took {:.2f} s'.format(teffend-teffstart))
        return efficiency

    def compute_kernels(self):
        if 'GCph' in self.observables:
            self.GC     = [interp1d(self.z, self.galaxy_kernel(self.z, ind), kind='cubic') for ind in self.binrange]
        ...
        if 'WL' in self.observables:
            ...
            self.WL         = [interp1d(self.z,self.lensing_kernel(self.z,ind)[0], kind='cubic') for ind in self.binrange]
            self.WL.insert(0,None)
            # Sakr Fix June 2023
            self.WL_IA         = [interp1d(self.z,self.lensing_kernel(self.z,ind)[1], kind='cubic') for ind in self.binrange]
    ...
    def genwindow(self,z,obs,i):
        ...
        if obs == 'GCph':
            ...
            win = self.GC[i](z)*bz
            # Sakr Fix June 2023
            # just filling it with zeros to keep the vectorisation possible later 
    ...
    def computecls(self):
        ...
        if self.feed_lvl > 1: print('    Computing Cls integral for {}'.format(self.observables))
        ...
        #PYTHONIZE THIS HORRIBLE THING
        for obs1,obs2,bin1,bin2 in product(self.observables, self.observables, self.binrange, self.binrange):

            clinterp = self.clsintegral(obs1,obs2,bin1,bin2,hub)

            finalcls = np.zeros((len(full_ell)))
            for ind,lval in enumerate(full_ell):
                if (cfg.specs['lmin_'+obs1]<=lval<=cfg.specs['lmax_'+obs1]) and (cfg.specs['lmin_'+obs2]<=lval<=cfg.specs['lmax_'+obs2]):
                    finalcls[ind] = clinterp(lval)

            cls[obs1+' '+str(bin1)+'x'+obs2+' '+str(bin2)] = finalcls

            tbin = time()
            if self.feed_lvl > 2: print('')
            if self.feed_lvl > 2: print('    ...{} {} x {} {} done in {:.2f} s'.format(obs1,bin1,obs2,bin2,tbin-tcell))
            tcell = tbin

        tend = time()
        if self.feed_lvl > 1: print('')
        if self.feed_lvl > 1: print('    Cls integral computation took {:.2f} s'.format(tend-tstart))
    ...
    def clsintegral(self,obs1,obs2,bin1,bin2,hub):
        ...
        intgn  = (self.sqrtPell[obs1]*self.genwindow(self.z,obs1,bin1)[0][:,np.newaxis]/np.sqrt(hub)[:,np.newaxis]+self.sqrtPell[obs1+'_IA']*self.genwindow(self.z,obs1,bin1)[1][:,np.newaxis]/np.sqrt(hub)[:,np.newaxis])*(self.sqrtPell[obs2]*self.genwindow(self.z,obs2,bin2)[0][:,np.newaxis]/np.sqrt(hub)[:,np.newaxis]+self.sqrtPell[obs2+'_IA']*self.genwindow(self.z,obs2,bin2)[1][:,np.newaxis]/np.sqrt(hub)[:,np.newaxis])
