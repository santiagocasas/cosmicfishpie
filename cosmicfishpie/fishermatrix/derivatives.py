...
class derivatives:
    ...
    def derivative_forward_4pt(self):
        ...
        for par in self.freeparams:
            ...
            for ffstep in fwdlist:
                upt.time_print(feedback_level=self.feed_lvl, min_level=2,
                               text='^^^ Computing observable at parameter {:s} with value: {:.6f} and stepsize: {:.4f}'\
                                .format(par, ffstep[par], (ffstep[par]-fwd_0[par]) )
                                )
    ...
    def derivative_stem(self):
        ...
        def adaptive_eps(param_eps):
            if self.external_settings != None:
                eps_v = np.array(self.external_settings['eps_values'])
                d_eps = np.concatenate([-eps_v[::-1],eps_v])
            else:
                d_eps = np.linspace(-param_eps*mult_eps_factor, param_eps*mult_eps_factor, numstem)
        ...
        for par in self.freeparams:
            ...
            if 'GCph' in self.observables or 'WL' in self.observables:
                for key in obs_mod[0]:
                    #WARNING: THIS WORKS FOR NOW, BUT OTHER THINGS HAVE TO BE ADDED TO THE IF FOR OTHER OBS
                    if key == 'ells':
                        ...
                        for ind in range(len(dpar['ells'])):
                            ...
                            while residuals > threshold:
                                fit = np.polyfit(tempstep,[obs_mod[step][key][ind] for step in range(len(tempstep))],1,full=True)
                                residuals = fit[1]
                                if residuals > threshold: tempstep = tempstep[1:-1]
                                counter += 1
    ...
    def derivative_poly(self):
        ...
        for par in self.freeparams:
            if self.fiducial[par] != 0.:
               stepsize = np.linspace(-self.fiducial[par]*self.freeparams[par],self.fiducial[par]*self.freeparams[par],numpoints)
            else:
            ...
            for key in obs_mod[0]:
                ...
                if key == 'ells':
                   ...
                   temp = []
                   for ind in range(len(dpar['ells'])):
                       fit = np.polyfit(stepsize,[obs_mod[step][key][ind] for step in range(len(stepsize))],4)
                       temp.append(4*fit[0]*fidpar**3*+3*fit[2]*fidpar**2+2*fit[3]*fidpar+fit[4])
