...
class SpectroCov:
    ...
    def volume_bin(self, zi, zj):
        ...
        sphere_vol = (4 * np.pi / 3) * \
            (pow((1 + zj) * d2, 3) - pow((1 + zi) * d1, 3))
        vol = sphere_vol * rad_to_area
        return vol

    def d_volume(self, ibin):
        return self.volume_bin(
            self.global_z_bins[ibin], self.global_z_bins[ibin + 1])
    ...
    def P_noise_21(self, z, k, mu, temp_dim=True, beam_term=False):
        if not temp_dim:
            ...
            temp = 1
        pref = ((2 * np.pi * self.pk_obs.fsky_IM) /
                (self.pk_obs.f_21 * self.pk_obs.t_tot))
        cosmo = ((1 + z)**2 * self.pk_obs.cosmo.comoving(z)**2) / \
            self.pk_obs.cosmo.Hubble(z)
...
class SpectroDerivs:
    ...
    def initialize_obs(self, allpars):
        ...
        spectrobiaspars = dict((k, allpars[k])
                               for k in self.fiducial_spectrobiaspars)
        PShotpars = dict((k, allpars[k]) for k in self.fiducial_PShotpars)
        spectrononlinearpars = dict(
            (k, allpars[k]) for k in self.fiducial_spectrononlinearpars)

        if 'I' in self.bias_samples:
            IMbiaspars = dict((k, allpars[k])
                              for k in self.fiducial_IMbiaspars)
        if 'I' in self.bias_samples:
    ...
    def get_obs(self, allpars):
        ...
        for ii, zzi in enumerate(self.z_array):
            if 'I' in self.bias_samples:
                result_array[ii] = self.pobs.lnpobs_IM(
                    zzi, self.pk_kmesh, self.pk_mumesh)
            elif 'g' in self.bias_samples:
                result_array[ii] = self.pobs.lnpobs_gg(
                    zzi, self.pk_kmesh, self.pk_mumesh)
        return result_array
    ...
    def exact_derivs(self, par):
        if 'Ps' in par:
            deriv = dict()
            for ii, zzi in enumerate(self.z_array):
                pgg_obs = self.pobs.observed_Pgg(
                    zzi, self.pk_kmesh, self.pk_mumesh)
